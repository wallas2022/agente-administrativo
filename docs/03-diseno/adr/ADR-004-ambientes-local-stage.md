...
- **Excepción: motor LLM nativo en local (2026-09-24, ver docs/04-pruebas/resultados/local-L1.md).** La inferencia dentro de Docker Desktop/WSL2 en la máquina de desarrollo usada resultó ~100x más lenta que el mismo modelo corriendo nativo en Windows (0.06–0.08 tokens/s vs 7 tokens/s medidos) — no es un problema de límites de CPU/RAM del contenedor, es la capa de virtualización de WSL2. Por eso, **solo en local**, `LLM_BASE_URL` apunta al Ollama nativo del host (`http://host.docker.internal:11434`) en vez del contenedor `ollama` del stack; el contenedor `ollama` se deja corriendo sin modelos cargados, únicamente por paridad de servicios con stage. En stage, donde Docker corre nativo sobre Linux (sin esa penalización de virtualización), el contenedor `ollama` sí es el motor real — esta excepción no aplica.
